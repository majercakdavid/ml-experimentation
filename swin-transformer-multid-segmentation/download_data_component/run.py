...
def main():
    ...
    #     metadata["test"] = [path.split("/")[1] for path in metadata["test"]]
    #     metadata["training"] = [
    #         {
    #             "image": sample["image"].split("/")[1],
    #             "label": sample["label"].split("/")[1],
    #         }
    ...
    #     syn, "syn3193805", "./temp_dataset", ifcollision="keep.local"
    # )
    with zipfile.ZipFile(dataset.path, "r") as zip_ref:
        zip_ref.extractall("./temp_unzipped_dataset")

    # # test_data = [file.path for file in files if file.cacheDir.split('/')[-1] == "averaged-testing-images"]
    # train_images = [file.path for file in files if file.cacheDir.split('/')[-1] == "averaged-training-images"]
    # train_labels = [file.path for file in files if file.cacheDir.split('/')[-1] == "averaged-training-labels"]

    # train_images = list(sorted(train_images))
    # train_labels = list(sorted(train_labels))
