...
download_data_component = load_component(
    source=os.path.join(COMPONENTS_FOLDER, "download_data_component", "spec.yaml")
)

train_component = load_component(
    source=os.path.join(COMPONENTS_FOLDER, "train_component", "spec.yaml")
)
...
def train_swin_multid_segmentation():
    ...
    train_component_step.distribution.process_count_per_instance = 4
